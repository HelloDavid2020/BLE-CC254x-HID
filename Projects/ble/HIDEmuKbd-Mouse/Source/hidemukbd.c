...
static void hidEmuKbdSendReport( uint8 keys, uint8 keycode );
static void hidEmuKbdSendMouseReport( uint8 buttons ,char x,char y);
...
static void hidEmuKbd_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  {
    // 按下鼠标左键
    hidEmuKbdSendMouseReport( MOUSE_BUTTON_LEFT, 0, 0 );
  }

  if ( keys & HAL_KEY_SW_2 )
  {
    // 按下鼠标右键
    hidEmuKbdSendMouseReport( MOUSE_BUTTON_RIGHT, 0, 0 );
  }
  
  if ( keys == HAL_KEY_NONE )
  {
    // 释放按键
    hidEmuKbdSendMouseReport( MOUSE_BUTTON_NONE, 0, 0 );
  }
