...
static void hidEmuKbd_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  {
    // 按下按键 Alt+F5  播放器 暂停
    hidEmuKbdSendReport( 0x04, HID_KEYBOARD_F5 );
  }

  if ( keys & HAL_KEY_SW_2 )
  {
    // 按下按键 Alt+Right 播放器 下一首
    hidEmuKbdSendReport( 0x04, HID_KEYBOARD_RIGHT_ARROW );
  }
  
  if ( keys == HAL_KEY_NONE )
  {
    // 释放按键
    hidEmuKbdSendReport( 0x00, HID_KEYBOARD_RESERVED );
  }
